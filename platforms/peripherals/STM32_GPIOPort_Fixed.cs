// Derived from 1.15.0 STM32_GPIOPort.cs
//
// Fix requirement to maintain GPIOx_OTYPER state
// Fix latching of ODR and avoidance of corrupting IDR state on ODR/BSRR writes
// TODO: Implement GPIOx_LCKR
//
...
namespace Antmicro.Renode.Peripherals.GPIOPort
{
    ...
    public class STM32_GPIOPort_Fixed : BaseGPIOPort, IDoubleWordPeripheral, ILocalGPIOReceiver
    {
        public STM32_GPIOPort_Fixed(IMachine machine, uint modeResetValue = 0, uint outputSpeedResetValue = 0, uint pullUpPullDownResetValue = 0,
            uint numberOfAFs = 16) : base(machine, NumberOfPins)
        {
            ...
            latchedOutState = new bool[NumberOfPins];
            outMask = new bool[NumberOfPins];
        ...
        public override void Reset()
        {
            ...
            for(var i = 0; i < NumberOfPins; i++)
            {
                latchedOutState[i] = false; // ODR resets to 0x00000000
                ...
                // outputType.Value = 0x00000000;
                // ConfigurationLock = 0x00000000
            }
        }
        ...
        {
            this.Log(LogLevel.Debug, "STM32_GPIOPort:OnGPIO: number {0} value {1}", number, value);
            base.OnGPIO(number, value); // Will set State[number] accordingly
        ...
        private void WritePin(int number, bool value)
        {
            ...
            {
                this.Log(LogLevel.Debug, "STM32_GPIOPort:WritePin: number {0} value {1}", number, value);
            }
            State[number] = value;
            Connections[number].Set(value);
        }
        ...
        {
            //this.Log(LogLevel.Debug, "STM32_GPIOPort:WriteState: value 0x{0:X}", value);
            for(var i = 0; i < NumberOfPins; i++)
            {
                var state = ((value & 1u) == 1);
                //this.Log(LogLevel.Debug, "STM32_GPIOPort:WriteState: prev latchedOutState[{0}] {1} state {2} outMask[{0}] {3}", i, latchedOutState[i], state, outMask[i]);
                latchedOutState[i] = state;
                if (outMask[i])
                {
                    WritePin(i, state);
                }
        ...
        {
            //this.Log(LogLevel.Debug, "ChangeMode: number {0} newMode {1}", number, newMode);
            mode[number] = newMode;
            if (Mode.Output == newMode)
            {
                outMask[number] = true;
                if (latchedOutState[number] != State[number])
                {
                    WritePin(number, latchedOutState[number]);
                }
            }
            else
            {
                outMask[number] = false;
            }
        ...
        private DoubleWordRegisterCollection CreateRegisters()
        {
            var registersMap = new Dictionary<long, DoubleWordRegister>
            {
                ...
                {(long)Registers.OutputType, new DoubleWordRegister(this)
                    .WithValueField(0, 16, out outputType, name: "OT")
                ...
                {(long)Registers.OutputData, new DoubleWordRegister(this)
                    .WithValueField(0, 16,
                                    valueProviderCallback: _ => BitHelper.GetValueFromBitsArray(latchedOutState),
                                    writeCallback: (_, val) => WriteState((ushort)val),
                                    name: "ODR")
                    .WithReservedBits(16, 16)
                },
        ...
        // This is just to hold state, since we do not need to
        // simulate push-pull vs open-drain for output pins:
        private IValueRegisterField outputType;

        // We need to track the latched ODR state as distinct from the
        // visible IDR State since we should not affect
        // input/AF/Analog pins when writing ODR values.
        private bool[] latchedOutState;
        ...
        {
            public GPIOAlternateFunction(STM32_GPIOPort_Fixed port, int pin)
            {
                this.port = port;
                this.pin = pin;
            ...
            private readonly STM32_GPIOPort_Fixed port;
